...
"""                                  Автоперевірка 5
                        ...
                        Теорія автоперевірки 5
                        
Not all Python objects can be serialized. For example, you can't serialize a file descriptor or a system resource. 
What if you have a class whose object you want to package using a pickle, but it has attributes that are not serializable? 
In this situation, you can use the magic methods that control serialization and deserialization with pickle.

The magic __getstate__ method is called when pickle tries to get a representation of an object as a byte string. In a normal 
implementation, __getstate__ returns a __dict__ dictionary where all the attributes of the class are stored. But you can 
change this method.
...
class Reader:
    ...
    def __getstate__(self):
        ...
        attributes['fh'] = None
        return attributes
In this example, the Reader class can be serialized, and no errors will occur due to the inability to pack the file descriptor.


                        Завдання до автоперевірки 5
...
class Contacts:
    ...
    def __getstate__(self):
        attributes = self.__dict__.copy()
        if 'count_save' not in attributes:
            attributes['count_save'] = 0
...
"""                                  Автоперевірка 6
                        ...
                        Теорія автоперевірки 6
                        
This magic method takes as input a dictionary unpacked from a file or byte string. The default behavior is to write the resulting value to self.__dict__. Let's modify the Reader class so that it can continue reading from the same place after unpacking.
...
class Reader:
    ...
    def __setstate__(self, value):
        ...
        self.fh.seek(value['position'])
                                ...
                                Завдання до автоперевірки 6
                                
Let's continue to extend the example from the previous task. Add the is_unpacking attribute to the Contacts class, 
which should be set to False by default. Implement the __setstate__ magic method for the Contacts class. When unpacking 
an instance of the class, the method should change the value of the is_unpacking attribute to True. Thus, this property 
will determine that the class instance was obtained as a result of unpacking.
...
"""                                  Автоперевірка 8
                        ...
                        Теорія автоперевірки 8
                        
n cases where we need new objects to be created at any nesting level, rather than references to existing ones, the copy package has a deepcopy function. This function creates new objects recursively.
...
print(my_list)  # [1, 2, {1: 'a'}]
                        ...
                        Завдання до автоперевірки 8


As you have already understood, for the Contacts class, creating a surface copy of a class instance will not succeed 
because we have a contacts attribute. This attribute is a list of object instances of the Person class, which means 
that all of them will be passed by reference. Therefore, you need to use the deepcopy method from the copy package.

For the Contacts class, implement the copy_class_contacts function. As a parameter, it takes an instance of the 
Contacts class and returns a deep copy of the object using the deepcopy function from the copy package.
